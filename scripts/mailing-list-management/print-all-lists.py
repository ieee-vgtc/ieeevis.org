"""
Print all mailing lists and users.
"""
...
def main():
    ...
    for r in results['groups']:
        if not r['email'].endswith('ieeevis.org'):
            continue
        print(r['email'])
        ms = service.members().list(groupKey=r['email']).execute()
        for m in ms.get('members', [{'email':'<none>'}]):
            print("   ",m['email'])
