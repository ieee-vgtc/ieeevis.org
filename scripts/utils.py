...
def find_files():
    ...
    return subprocess.run(['find', '.', '-type', 'f'], check=True, stdout=subprocess.PIPE, universal_newlines=True).stdout.strip().split('\n')
...
def diff_local_remote_buckets(local, remote):
    ...
    same = [k for (k, v) in local.items()
            if remote.get(k, {}).get("ETag", "")[1:-1] == v.get('ETag')]
    diff = [k for (k, v) in local.items()
