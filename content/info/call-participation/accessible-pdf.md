...
### For LaTeX Users
1. Download the latest template or use the Overleaf template.
2. **Use appropriate commands to create sections**, ordered/unordered lists, figures, tables, equations, etc.
3. **Add figure descriptions (alternative text) to all figures.** For each image in your document, you can use the `\Description{...}` tag to add an image description. For example:
```
\begin{figure}
